...
摘要：
本周主要工作是基于刘老师发的repo，整理了staged线性模型和parallel并行模型的代码，并使用600k样本训练，200k样本测试。
- 相关代码在dev branch。

模型的表现上，暂时还是parallel三个任务并行训练的效果更好（rejection、jet confusion trace、track confusion trace）。

然而，进一步分析staged线性模型不同任务表现的时候，发现vertex encoder h2的输出refine处理后几乎都变成了0，进而导致vtx_weight也几乎都为零。
- 尽管训练可以正常进行，但是refine归零的原因、如何合理使用vertex encoder h2的信息是下一步计划分析的内容。

## 代码准备

基于刘老师发的repo，把代码模块化重构了一下传到github上，放到了dev branch。
...
两个模型的参数量均约为55k，均为32维的track embedding，约55k参数量。具体配置在configs文件夹。
...
- staged模型的vertex任务的loss在20轮左右发生骤降，且几乎主导了整个loss的下降。这是串行的第二个任务，是直接回归拟合Lxy的，即使加上dz，或者使用minimal的vertex特征也一样。
  - 但是100轮后，staged的loss好像还能降，考虑使用更多轮数的训练
