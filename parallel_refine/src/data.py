...
TRACK_AUX_FIELDS = (
    ...
    "ftagTruthOriginLabel", "ftagTruthVertexIndex",
)
...
def _allocate(n: int, k: int, n_features: int) -> dict[str, np.ndarray]:
    return {
        "X": np.empty((n, k, n_features), dtype=np.float32),
...
def load_processed_split(
    ...
    track_read_fields = _unique((*TRACK_AUX_FIELDS, *track_fields))
    with h5py.File(config.train_file, "r") as handle:
        jets = handle["jets"]
        tracks = handle["tracks"]
        n_rows = _row_count(jets, JET_FIELDS[0])
        ...
        _require_fields(jets, JET_FIELDS, "jets")
        ...
        outputs = _allocate(len(indices), output_k, len(track_fields))
        out_pos = 0
        ...
        for begin, end, start, stop in _block_slices(
            ...
            jet_block = _read_fields(jets, JET_FIELDS, start, stop)
            ...
            vertex_index = track_block[
                "ftagTruthVertexIndex"][local].astype(np.int64)[rows, order]
            outputs["origin"][destination] = np.where(top_mask, origin, -1)
            valid_pair = top_mask[:, :, None] & top_mask[:, None, :]
    ...
    result = {name: value[:out_pos] for name, value in outputs.items()}
