"""Production data preparation for SerialFlavour.

This module avoids the expensive ``compound_dataset[field][indices]`` access
pattern.  Real OpenData
...
def _allocate_outputs(n: int, top_k: int, n_fields: int,
    ...
    return {
        ...
        # Stored unconditionally so pair-capable models can reuse the cache.
...
def load_tracks(path, idx, flavour_to_label, track_fields,
                ...
                *, force: bool = False, block_rows: int | None = None,
                ...
                data_seed: int = 42):
    """Load and pre-process a selected, increasing sequence of jet indices.

    Output keys, shapes, dtypes, row order, top-K ordering, padding semantics,
    pair targets and cache filenames follow the project's established cache
    contract.
...
class JetDataset(Dataset):
    """Expose the eight-item batch interface consumed by project models."""
...
def _split_metadata_cache_path(config):
    ...
    identity = (
        ...
        f"{_JET_FLAVOUR_FIELD}\0{JET_EVENT_FIELD}\0{SPLIT_VERSION}")
...
def create_dataloaders(config, device, *, force=False, block_rows=None,
                       progress=False):
    """Build event-disjoint train, validation and test loaders."""
...
# ---------------------------------------------------------------------------
# Event-level train/validation/test splitting and split manifests.
# ---------------------------------------------------------------------------
