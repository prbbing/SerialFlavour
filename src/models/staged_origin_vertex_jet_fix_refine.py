...
Stage 2 (encoder 2) — differentiable origin-gated secondary-vertex fit.
    ...
    soft_embed + vertex_features → input_proj2 → Encoder2 → bounded weight residuals.
    For each vertex leg k:
        gate_i,k = sigmoid((p_origin_k(track_i) - 0.5) / T)
        Δw_i,k^lxy, Δw_i,k^dz = small bounded corrections from encoder2
        w_i,k^lxy = clamp(gate_i,k + Δw_i,k^lxy, 0, 1)
        w_i,k^dz  = clamp(gate_i,k + Δw_i,k^dz,  0, 1)
...
class StagedOriginVertexJetTransformer(nn.Module):
    def __init__(self, in_dim, d_model, n_heads, n_layers, d_ffn, dropout,
        ...
        # Small bounded residuals to refine Stage-2 fit weights without
        # overpowering the origin-derived gate.
    ...
    def forward(self, x, mask):
        ...
        # Origin-derived gate remains the primary assignment signal.
        ...
        # Use separate bounded residual corrections for transverse and
        # longitudinal fits. This lets encoder2 contribute fit-quality
        # information without collapsing the gate multiplicatively.
        w_lxy = torch.clamp(gate + delta_w_lxy, min=0.0, max=1.0) * mask_f
        w_dz  = torch.clamp(gate + delta_w_dz,  min=0.0, max=1.0) * mask_f

        # Stage 3 currently expects one per-track, per-leg weight tensor.
        # Use the Lxy fit weights as the canonical weight representation.
        vtx_weight = w_lxy
        ...
        return {
            ...
            "vtx_weight":        vtx_weight,          # (B, K, L) legacy: uses w_lxy
            "leg_origin_probs":  leg_origin_probs,    # (B, K, L)
            "gate":              gate,                # (B, K, L)
            "refine":            delta_w_lxy,         # legacy diagnostics alias
            "delta_w_lxy":       delta_w_lxy,         # (B, K, L)
            "delta_w_dz":        delta_w_dz,          # (B, K, L)
            "w_lxy":             w_lxy,               # (B, K, L)
            "w_dz":              w_dz,                # (B, K, L)
...
def build_staged_origin_vertex_jet_fix_refine(config):
    """Factory: Config → fix-refine StagedOriginVertexJetTransformer."""
    return StagedOriginVertexJetTransformer(
        ...
        delta_w_amp=config.delta_w_amp,
    )
