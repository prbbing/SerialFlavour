"""
Staged three-encoder transformer jet-flavour classifier (fix_refine + fixed dz).

Differs from staged_origin_vertex_jet in Stage 2 weighting:
  - Origin gate + bounded additive delta_w residuals (per-coord)
  - w_lxy = clamp(gate + delta_w_lxy, 0, 1)  for transverse fit
  - w_dz  = clamp(gate + delta_w_dz,  0, 1)  for longitudinal fit
...
class StagedOriginVertexJetTransformerFixRefineFixDz(nn.Module):
    def __init__(self, in_dim, d_model, n_heads, n_layers, d_ffn, dropout,
                 ...
                 vertex_leg_origin_matrix, gate_temp=0.1, delta_w_amp=0.5,
        ...
        # Stage 2 — fix_refine: bounded additive per-coord delta weights
        ...
        self.vertex_delta_head_lxy = nn.Sequential(
            nn.LayerNorm(d_model), nn.Linear(d_model, self.n_vertex_legs))
        self.vertex_delta_head_dz = nn.Sequential(
            nn.LayerNorm(d_model), nn.Linear(d_model, self.n_vertex_legs))
    ...
    # ====================================================================
    def _vertex_fit_two_step(self, x, w_lxy, w_dz):
        B = x.shape[0]
        ...
        if self.fit_dz:
            ...
            inv_var_z0 = w_dz / z0st_unc.pow(2)
    ...
    # _vertex_fit_old_dz
    # ====================================================================
    def _vertex_fit_old_dz(self, x, w_lxy, w_dz):
    ...
    # _vertex_fit_3dwls
    # ====================================================================
    def _vertex_fit_3dwls(self, x, w_lxy, w_dz):
        B, K, L = w_lxy.shape
        device  = x.device
        ...
        lxy_pred = w_lxy.new_zeros(B, L)
        dz_pred  = w_lxy.new_zeros(B, L)
        flight_phi = w_lxy.new_zeros(B, L)
        delta_phi  = w_lxy.new_zeros(B, K, L)

        for leg in range(L):
            w_t = w_lxy[:, :, leg]
            w_l = w_dz[:, :, leg]

            w_d = w_t / d0_unc.pow(2)
            sin_mask = (sin_th.abs() >= MIN_SIN_THETA).float()
            w_z = w_l / z0st_unc.pow(2) * sin_mask
    ...
    def forward(self, x, mask):
        ...
        delta_w_lxy = self.delta_w_amp * torch.tanh(self.vertex_delta_head_lxy(h2))
        delta_w_dz  = self.delta_w_amp * torch.tanh(self.vertex_delta_head_dz(h2))

        w_lxy = torch.clamp(gate + delta_w_lxy, min=0.0, max=1.0) * mask_f
        w_dz  = torch.clamp(gate + delta_w_dz,  min=0.0, max=1.0) * mask_f

        if self.vertex_fit_method == "wls_3d":
            lxy_pred, dz_pred, flight_phi, delta_phi = \
                self._vertex_fit_3dwls(x, w_lxy, w_dz)
        elif self.vertex_fit_method == "old_dz":
            lxy_pred, dz_pred, flight_phi, delta_phi = \
                self._vertex_fit_old_dz(x, w_lxy, w_dz)
        else:
            lxy_pred, dz_pred, flight_phi, delta_phi = \
                self._vertex_fit_two_step(x, w_lxy, w_dz)

        # Canonical vtx_weight for Stage 3 and export
        vtx_weight = w_lxy
        refine = delta_w_lxy
...
def build_staged_origin_vertex_jet_fix_refine_fix_dz(config):
    return StagedOriginVertexJetTransformerFixRefineFixDz(
        ...
        gate_temp=config.gate_temp, delta_w_amp=config.delta_w_amp,
