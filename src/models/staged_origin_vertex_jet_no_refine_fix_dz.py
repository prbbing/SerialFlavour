"""
Staged two-encoder transformer jet-flavour classifier (no refine, fixed dz).

Differs from staged_origin_vertex_jet in two ways:
  1. Encoder2 (refine head) is removed — vtx_weight = gate * mask_f only.
  2. Vertex fitting uses fixed-dz formulas (two_step or wls_3d).
...
class StagedOriginVertexJetTransformerNoRefineFixDz(nn.Module):
...
def build_staged_origin_vertex_jet_no_refine_fix_dz(config):
    return StagedOriginVertexJetTransformerNoRefineFixDz(
